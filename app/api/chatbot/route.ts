...
const FALLBACK_RESPONSES: Record<string, string> = {
  ...
  graphic: "Yes! We build high-converting graphic & kirana storefronts with instant WhatsApp reordering, neighborhood delivery zone setups, and local SEO.",
  dairy: "Our uiux and dairy delivery websites support subscription frequencies (daily, alternate), automated WhatsApp order summaries, and recurring customer management.",
...
function getLocalFallback(query: string): { reply: string; intent: string; confidence: number } {
  ...
  if (q.includes("graphic") || q.includes("store") || q.includes("kirana")) {
    return { reply: FALLBACK_RESPONSES.graphic, intent: "graphic", confidence: 0.91 };
  }
  if (q.includes("uiux") || q.includes("dairy")) {

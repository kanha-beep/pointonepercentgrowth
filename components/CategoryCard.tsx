    ...
    <article className={`rounded-[32px] border border-slate-950/10 p-7 shadow-soft backdrop-blur-xl ${accentClasses[item.accent] || "bg-white/75"}`}>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3.5">
        {item.websites.map((website) => (
          <span key={website} className="rounded-[14px] border border-slate-950/10 bg-white/75 px-3 py-2 text-[0.86rem] text-slate-950">{website}</span>
        ))}
      </div>
      {item.packages ? (
        <div className="mt-[18px] grid gap-2.5">
          {item.packages.map((pkg) => (
            <div key={pkg.tier} className="flex justify-between gap-3 rounded-2xl border border-slate-950/10 bg-white/75 px-3.5 py-3">
              <strong className="text-[0.88rem]">{pkg.tier} - {pkg.pages} pages</strong>

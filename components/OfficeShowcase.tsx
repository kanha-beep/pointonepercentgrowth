...
import type { ContactInfo, SiteContent } from "@/lib/site";

export default function OfficeShowcase({ office, contact }: { office: SiteContent["office"]; contact: ContactInfo }) {
  return (
    <section className="px-0 py-10 pb-[88px] max-[720px]:pb-[72px] max-[720px]:pt-8">
      <div className="mx-auto grid w-[min(1180px,calc(100%-40px))] items-start gap-7 lg:grid-cols-[1.05fr_0.95fr] max-[720px]:w-[min(100%-28px,1180px)]">
        <Reveal>
          <SectionHeading eyebrow="Office" title={office.title} description={office.description} />
          <div className="space-y-2 leading-8 text-slate-500">
            <p><strong className="text-slate-950">{contact.officeName}</strong></p>
              ...
              <div className="relative min-h-[300px] overflow-hidden p-7">
                ...
                <div className="relative z-10 grid gap-3.5">
                  {office.bullets.map((bullet) => (
                    <article key={bullet} className="grid grid-cols-[18px_1fr] gap-3 rounded-[18px] bg-white/10 p-[18px] text-white/90">
